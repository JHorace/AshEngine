...
pub fn check_validation_layer_support(entry: &ash::Entry, num_validation_layers: &u32, validation_layers: &* const * const c_char) -> bool
{
    ...
    for i in 0..*num_validation_layers
    {
        ...
        unsafe {
            if !available_layers.iter().any(|&j| CStr::from_ptr(j.layer_name.as_ptr()) == CStr::from_ptr(*validation_layers.offset(i as isize)))
            {
                return false;
            }
        }
