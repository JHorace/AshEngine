...
{
    pub fn new(instance: &ash::Instance, device: &ash::Device, physical_device: &PhysicalDevice, max_objects: u64, num_frames: usize) -> InstanceManager
    {
        ...
        InstanceManager{ instances_: HashMap::new(), uniform_buffers_: buffers, curr_instance_id_: 0 }
    }
    ...
    pub fn create_instance(&mut self, mesh_id: u64, descriptor_set: DescriptorSet) -> u64
    {
        ...
        self.instances_.insert(instance_id, (instance, descriptor_buffer_info));
    ...
    {
        let (instance, ..) = self.instances_.get_mut(instance_id).expect("No such instance");
        instance.update(ubo);
    }
