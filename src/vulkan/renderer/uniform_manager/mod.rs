...
use super::super::geometry::scene::{Instance, InstanceID};
use ash;
...
impl UniformManager
{
    pub fn new(instance: &ash::Instance, device: &ash::Device, physical_device: &PhysicalDevice, max_instances: u64, num_frames: u32, ubo_size: u64) -> UniformManager
    {
        ...
        UniformManager{ uniform_buffers_: buffers, ubo_size_: ubo_size}
    }

    pub fn update_uniforms(& mut self, curr_frame: u32, instances: &Vec<Instance>) -> Vec<DescriptorBufferInfo>
    {
        ...
        for (i, instance) in instances.iter().enumerate()
        {
          let data_ptr : * const c_void = & instance.transform_ as * const _ as * const c_void;
            unsafe{self.uniform_buffers_[curr_frame as usize].copy_from_data(data_ptr, self.ubo_size_ , self.ubo_size_ * i as u64)};
