...
impl Renderer
{
    pub unsafe fn new(instance: &ash::Instance, device: &ash::Device, physical_device: &physical_device::PhysicalDevice, surface: &surface::Surface) -> Renderer
    {
        ...
        Renderer{swapchain_: swapchain, instance_manager_: instance_manager, scene_manager_: scene_manager, command_dispatch_: command_dispatch, present_queue_: present_queue, descriptor_pool_: descriptor_pool, descriptor_sets_: vec![], render_finished_semaphores_: render_finished_semaphores, image_available_sempahores_: image_ready_semaphores, in_flight_fences_: in_flight_fences, current_frame_: 0, curr_descriptor_set_: 0, instances_loaded_: true, pipeline_: None, render_pass_: None }
    }
    ...
    fn update_descriptor_sets(&self, device: &ash::Device, curr_frame: usize)
    {
    ...
    pub unsafe fn draw_frame(&mut self, device: &ash::Device, geometry_manager: &geometry_manager::GeometryManager)
    {
        ...
        let wait_fences = [self.in_flight_fences_[self.current_frame_]];

        device.wait_for_fences(&wait_fences, true, std::u64::MAX);

        let (image_index, ready) = self.swapchain_.acquire_next_image(self.image_available_sempahores_[self.current_frame_]);

        let wait_semaphores = [self.image_available_sempahores_[self.current_frame_]];
        let wait_stages = [ash::vk::PipelineStageFlags::COLOR_ATTACHMENT_OUTPUT];
        let signal_semaphores = [self.render_finished_semaphores_[self.current_frame_]];
        ...
        device.queue_submit(self.present_queue_, &submit_infos, self.in_flight_fences_[self.current_frame_]);
