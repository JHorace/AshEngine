...
impl Buffer{
    ...
    pub fn new(instance: &ash::Instance, device: &ash::Device, physical_device: &physical_device::PhysicalDevice, size: ash::vk::DeviceSize, usage: ash::vk::BufferUsageFlags, required_properties: ash::vk::MemoryPropertyFlags) -> Buffer
    {
        ...
        let buffer_handle = unsafe{device.create_buffer(&buffer_create_info, None).expect("Could not create buffer")};

        let memory_requirements = unsafe{device.get_buffer_memory_requirements(buffer_handle)};

        let memory_type_index = physical_device.get_suitable_memory_type_index(instance, memory_requirements.memory_type_bits, required_properties);
        ...
        let device_memory_handle = unsafe { device.allocate_memory(&memory_allocate_info, None).expect("Could not allocate device memory for buffer") };
        unsafe { device.bind_buffer_memory(buffer_handle, device_memory_handle, 0) };
    ...
    pub unsafe fn map(&mut self, device: &ash::Device, offset: ash::vk::DeviceSize, size: ash::vk::DeviceSize) -> * mut c_void
    {
        self.mapped_memory_ = device.map_memory(self.device_memory_handle_, offset, size, ash::vk::MemoryMapFlags::empty()).expect("Could not map memory");
        self.mapped_memory_
    }
    ...
    pub unsafe fn copy_from_buffer(&mut self, device: &ash::Device, queue: ash::vk::Queue, command_buffer: ash::vk::CommandBuffer, src: &Buffer, copy_info: ash::vk::BufferCopy)
    {
        ...
        device.cmd_copy_buffer(command_buffer, src.buffer_handle_, self.buffer_handle_, &copy_infos);
        ...
        device.queue_submit(queue, &submit_info, ash::vk::Fence::null()).expect("Could not submit to queue");
    }
}