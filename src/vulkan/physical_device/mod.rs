...
{
    fn new(graphics: Option<usize>, graphics_compute: Option<usize>, compute: Option<usize>, transfer: Option<usize>) -> QueueFamilyIndices
    {
        ...
        QueueFamilyIndices{graphics_: graphics_u32, graphics_compute_: graphics_compute_u32, compute_: compute_u32, transfer_: transfer_u32}
    }
...
impl PhysicalDevice
{
    ...
    pub fn new(instance: &ash::Instance, physical_device: vk::PhysicalDevice) -> PhysicalDevice
    {
        ...
        let queue_family_indices = PhysicalDevice::select_queue_family_indices(instance, physical_device);
        PhysicalDevice { physical_device_handle_: physical_device, device_name_: device_name, queue_family_indices_: queue_family_indices}
    }

    fn select_queue_family_indices(instance: &ash::Instance, physical_device: vk::PhysicalDevice) -> QueueFamilyIndices
    {
        let queue_families = unsafe{instance.get_physical_device_queue_family_properties(physical_device)};

        let transfer = queue_families.iter()
            .position(|&queue_family|
                queue_family.queue_count > 0 &&
                    queue_family.queue_flags.contains(vk::QueueFlags::TRANSFER) &&
            !queue_family.queue_flags.contains(vk::QueueFlags::GRAPHICS));

        let compute = queue_families.iter()
            .position(|&queue_family|
                queue_family.queue_count > 0 &&
                    queue_family.queue_flags.contains(vk::QueueFlags::COMPUTE) &&
            !queue_family.queue_flags.contains(vk::QueueFlags::GRAPHICS));

        let graphics = queue_families.iter()
            .position(|&queue_family|
                queue_family.queue_count > 0 &&
                    queue_family.queue_flags.contains(vk::QueueFlags::GRAPHICS) &&
            !queue_family.queue_flags.contains(vk::QueueFlags::COMPUTE));

        let graphics_compute = queue_families.iter()
            .position(|&queue_family|
                queue_family.queue_count > 0 &&
                    queue_family.queue_flags.contains(vk::QueueFlags::GRAPHICS) &&
            queue_family.queue_flags.contains(vk::QueueFlags::COMPUTE));
    ...
    pub fn get_suitable_memory_type_index(&self, instance: &ash::Instance, type_bitmask: u32, require_properties: ash::vk::MemoryPropertyFlags) -> Option<u32>
    {
        let device_memory_properties = unsafe{instance.get_physical_device_memory_properties(self.physical_device_handle_)};

        for(i, memory_type) in device_memory_properties.memory_types.iter().enumerate()
        {
            if(type_bitmask & (1 << i)) > 0 && (memory_type.property_flags.contains(require_properties))
            {
    ...
    pub fn get_supported_format(&self, instance: &ash::Instance, candidate_formats: &[ash::vk::Format], tiling: ash::vk::ImageTiling, features: ash::vk::FormatFeatureFlags) -> Option<ash::vk::Format>
    {
        for &format in candidate_formats.iter()
        {
            let properties = unsafe{instance.get_physical_device_format_properties(self.physical_device_handle_, format)};
...
///
pub fn get_suitable_physical_devices(instance: &ash::Instance, num_device_extensions: &u32, device_extensions: &*const * const c_char) -> Vec<PhysicalDevice>
{
...
/// * 'physical_device' - the vulkan physical device handle
///
fn supports_extensions(instance: &ash::Instance, physical_device: vk::PhysicalDevice,  num_device_extensions: &u32, device_extensions: &*const * const c_char) -> bool
{
    let available_device_extensions = unsafe{instance.enumerate_device_extension_properties(physical_device).expect("Could not enumeration device extension properties")};

    for i in 0..*num_device_extensions
    {
        unsafe {
            if !available_device_extensions.iter().any(|&j| CStr::from_ptr(j.extension_name.as_ptr()) == CStr::from_ptr(*device_extensions.offset(i as isize)))
            {
                return false;
            }
        }
...
///
fn is_suitable(instance: &ash::Instance, physical_device: vk::PhysicalDevice, num_device_extensions: &u32, device_extensions: &*const * const c_char) -> bool
{
        return is_gpu(instance, physical_device) && supports_graphics_and_presentation(instance, physical_device) && supports_extensions(instance, physical_device, num_device_extensions, device_extensions)
}
