...
impl Scene
{
    pub fn new() -> Scene
    {
        Scene{ instances_: HashMap::new(), curr_instance_id_: 0, view_: Matrix4::identity(), projection_: Matrix4::identity() }
    }

    pub fn create_instance(&mut self, mesh_id: MeshID, transform: Option<Matrix4<f32>>) -> InstanceID
    {
        self.instances_.insert(self.curr_instance_id_, Instance::new(mesh_id, transform));
    ...
    {
        self.instances_.get_mut(instance_id).expect("No such instance")
    }
